...
function trackEvent(eventName, properties = {}) {
  try {
    // Call window.pendo directly — this is the real, confirmed-working object
    // (Novus's own auto-instrumentation PR expects this exact pattern too).
    // window.novus is kept as a fallback alias in case it's set elsewhere.
    if (typeof window !== 'undefined' && window.pendo && typeof window.pendo.track === 'function') {
      window.pendo.track(eventName, properties);
    } else if (typeof window !== 'undefined' && window.novus && typeof window.novus.track === 'function') {
